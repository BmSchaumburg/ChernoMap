...
def move_toward_target(dot, target_x, target_y):
    ...
    if distance > 0:  # Avoid division by zero
        step = random.uniform(0.5, 0.5)  # Control movement speed (500m movements)
...
def is_near_hotspot(dot, hotspot, radius=0.7):
    distance = ((dot["x"] - hotspot[0]) ** 2 + (dot["y"] - hotspot[1]) ** 2) ** 0.5
    return distance <= radius

# Function to update the simulation
def update(frame):
    global dots, death_positions
    ...
    for dot in dots:
        ...
        if dot["death_time"] is not None and current_time - dot["death_time"] < 25:
        ...
        # Move dot randomly or toward the goal
        visited_hotspots = []
    ...
    while len(new_dots) < 60:  # Keep 60 dots alive
        area = random.choice(spawn_areas)
        x = random.uniform(area[1], area[2]) if area[0] == 1 else area[0]
        y = area[0] if area[0] == 1 else random.uniform(area[1], area[2])
        new_dot = {"x": x, "y": y, "trail": [(x, y)], "last_death": None, "death_time": None}
        new_dots.append(new_dot)

    # Replace old dots with updated dots
    dots = new_dots

    # Draw dots
    for dot in dots:
        # Plot the trail for each dot
        trail_x, trail_y = zip(*dot["trail"])
        ax.plot(trail_x, trail_y, color="blue", linewidth=0.5, alpha=0.6)  # Trails with 60% opacity
        ax.plot(dot["x"], dot["y"], "yo", markersize=4)  # Dots are now yellow
